...
Поддерживает 24-битный BMP без сжатия и без таблицы цветов. Тип используемого `DIB header` - `BITMAPINFOHEADER`.
...
В таком случае в качестве значения пикселя, выходящего за границу, будет использовать значение ближайшего
...
Оптимизирован с помощью разделения матрицы на два одномерных фильтра и проходке каждого из них по изображению.

### Пикселизация (-pixelate intensity)

Разбивает изображение на квадратные пиксельные блоки с длиной стороны блока в intensity пикселей.
